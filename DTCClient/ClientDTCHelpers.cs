...
public partial class ClientDTC
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // Send the request
        GetHistoricalData(requestId, symbol, exchange, recordInterval, startDateTimeUtc, endDateTimeUtc, maxDaysToReturn, useZLibCompression, requestDividendAdjustedStockData, integer1);
    ...
    /// <param name="isParentOrder">
    ///     The Client will set this to 1 when the order is part of a bracket order.
    ///     This indicates that this is the parent order. A bracket order will consist of a SUBMIT_NEW_SINGLE_ORDER message followed by a SUBMIT_NEW_OCO_ORDER message. The Server will use IsParentOrder as a
    ///     flag to know that this message is a parent order.
    ///     The Server will hold onto this order until it receives the subsequent SUBMIT_NEW_OCO_ORDER message and then process all of the orders as one complete set.
    /// </param>
