        ...
        if (encodingRequest.Encoding != _currentEncoding)
        {
            switch (encodingRequest.Encoding)
            {
                ...
                case EncodingEnum.JsonCompactEncoding:
                    s_logger.Verbose("ClientHandler is rejecting the encoding request change to {Encoding}, encoding remains {_currentEncoding} in {ClientHandlerDTC}", encodingRequest.Encoding,
                        _currentEncoding, this);
                    break;
                default:
